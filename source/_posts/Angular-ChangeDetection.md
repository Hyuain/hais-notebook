...
如果想要了解更多关于 OnPush 脏检查机制，请查阅 [这篇文章](https://blog.angular-university.io/onpush-change-detection-how-it-works) 。
...
Zone 其实就是一个在多个 JavaScript 虚拟机执行回合之中幸存下来的执行上下文——这是一个可以用来为浏览器增加更多功能的很普遍的机制。Angular 内部使用 Zones 来触发脏检查，但 Zone 也可以用来做应用分析（比如内存占用、程序复杂度等）、跨越不同的 JavaScript 虚拟机回合进行长堆栈跟踪。

# 浏览器异步 API 支持

为了支持脏检查，下面这些常用的浏览器机制也被修改了：

- 所有的浏览器事件（click、mouseover、keyup 等）
- `setTimeout()` 和 `setInterval()`
- Ajax HTTP 请求

事实上，为了无感知地触发 Angular 脏检查，Zone.js 也修改了很多别的浏览器 API，比如 Websocket。可以看看 Zone.js 的 [一段测试代码](https://github.com/angular/zone.js/tree/master/test/patch) 看看现在支持了哪些 API。

这个机制的局限性在于：如果出于某些原因，Zone.js 不支持某个浏览器的异步 API，那么脏检查就不会被触发。比如 IndexedDB 的回调。

上面解释了脏检查是怎样触发的，但是触发之后他到底是怎样运作的呢？

# 脏检查树

在应用的启动阶段，每个 Angular 组件都与一个脏检查器建立关联。比如下面的 `TodoItem` 组件：
...
事实上，我们可以在运行时看到脏检查器长什么样子！我们只需要在 Todo class 里面加一些代码来在访问他的属性的时候 [打一个断点](https://github.com/jhades/blog.angular-university.io/blob/master/ng2-change-detection/src/todo.ts#L11) 。

到达断点的时候，我们可以跟着调用栈看到脏检查：
...
我们可以看到在脏检查器的代码中，`owner` 中的对象也被检查了，但是只检查了 `firstName`，并没有检查 `lastName`。

这是因为 `lastName` 并没有被 template 使用！——同样地，Todo 中的 `id` 属性也没有被检查。

对此，我们可以说：

> 默认情况下，Angular 脏检查是检查 template 表达式中的值是否变化。他会检查所有的组件。
...
想象一下如果 Angular 默认脏检查机制是基于组件输入的引用的比较（而不是默认的机制）。这样的话即便是像 TODO 这样简单的应用都需要一些技巧来创建：开发者需要非常小心地来创建一个新的 Todo，而不是只是更新属性。

但就像我们将要看到的那样，如果有必要的话，也可以自定义 Angular 的脏检查。

# 性能如何？

注意 TodoList 组件的脏检查器是怎样创建对 `todos` 属性的显式引用的。

另一个方法是动态地遍历组件的属性，让代码具有普遍性（generic）——而不是为了这个组件特异化（specific）。这样我们就不用在每次组件创建的时候都创建一个脏检查器了！所以应该怎么做呢？
...
用生成的特异化脏检查器的结果就是：会非常快、可预测和易读。

接下来我们来讨论一下性能，有办法可以优化脏检查吗？
...
因为 `OnPush` 不只是在组件输入变化的时候进行检查，组件触发的事件同样会触发脏检查。

正如 Victor Savkin 在他的博客中所说：

> 如果使用 OnPush 检查器的时候，框架会在他输入的属性变化的时候、他发出事件的时候、Observable 触发事件的时候，检查 OnPush 组件

尽管 `OnPush` 可能会使得性能更好，在使用可变对象时，他的引入会带来更高的复杂度。这会引发一些很难推理和重现的 BUG。但是
